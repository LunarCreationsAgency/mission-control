...
import {
  ArrowLeft, Calendar, User, Target, FolderKanban, Flag, Clock, Pencil,
  Trash2, Loader2, Check, X, Send, Bot, MessageSquare, RefreshCw,
  Sparkles, AlertCircle, GitBranch, ArrowUpRight,
} from "lucide-react";
...
const priorityConfig: Record<string, { bg: string; text: string; label: string; icon: typeof Flag }> = {
  low: { bg: "bg-[var(--info-subtle)]", text: "text-[var(--info)]", label: "Low", icon: Flag },
  medium: { bg: "bg-[var(--warning-subtle)]", text: "text-[var(--warning)]", label: "Medium", icon: Flag },
  high: { bg: "bg-[var(--secondary-subtle)]", text: "text-[var(--secondary)]", label: "High", icon: Flag },
...
const statusConfig: Record<string, { label: string; pill: string }> = {
  todo: { label: "To Do", pill: "status-pill-todo" },
  in_progress: { label: "In Progress", pill: "status-pill-in_progress" },
  review: { label: "Review", pill: "status-pill-review" },
  done: { label: "Done", pill: "status-pill-done" },
};
...
export default function TaskDetailPage() {
  ...
  const handleReject = useCallback(async () => {
    toastError("Use the feedback box to tell the agent what to fix.");
  }, [toastError]);
  ...
  const handleSendFeedback = useCallback(async () => {
    ...
    try {
      await createTaskComment(id, {
        task: id, author: "Dustin W", author_type: "user",
        comment_type: "feedback", content: feedbackText.trim(),
      });
      const updated = await getTaskComments(id);
      setComments(updated);
      setFeedbackText("");
      success("Feedback sent — agent will address it");
    } catch (e) {
  ...
  const handleReassign = useCallback(async () => {
    ...
    try {
      ...
      await updateTask(id, {
        assignee: newAssignee, status: "todo", updated: new Date().toISOString(),
      } as Record<string, unknown>);
      await createTaskComment(id, {
        task: id, author: "System", author_type: "system",
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-full p-8">
        ...
        <div className="h-8 w-64 bg-[var(--surface)] rounded mb-2" />
        <div className="h-4 w-48 bg-[var(--surface)] rounded mb-8" />
        <div className="flex-1 bg-[var(--surface)] rounded-md" />
      </div>
    );
  }
  ...
  if (!task) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-8">
        <AlertCircle className="h-10 w-10 text-[var(--foreground-quaternary)] mb-4" />
        <p className="text-[var(--foreground-secondary)] font-medium">Task not found</p>
        <Link href="/tasks" className="mt-4 text-sm text-[var(--primary)] hover:underline">
          ← Back to Tasks
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="flex-shrink-0 border-b border-[var(--border)] px-8 py-5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2.5 mb-2">
              ...
              {task.revision_count && task.revision_count > 0 && (
                <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-[var(--agent-light)] bg-[var(--agent-subtle)] px-2 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {!deleteConfirm ? (
              <button
                ...
                className="flex items-center gap-1.5 rounded-md border border-[var(--border)] px-2.5 py-1.5 text-xs font-medium text-[var(--foreground-tertiary)] hover:bg-[var(--destructive-subtle)] hover:border-[var(--destructive-subtle)] hover:text-[var(--destructive)] transition-colors"
              >
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="rounded-md border border-[var(--border)] px-2.5 py-1.5 text-xs text-[var(--foreground-secondary)] hover:bg-[var(--surface-hover)] transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={deleting}
                  className="flex items-center gap-1.5 rounded-md bg-[var(--destructive)] px-2.5 py-1.5 text-xs font-medium text-white hover:opacity-90 disabled:opacity-50 transition-opacity"
                >
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {/* Review Banner */}
          {isReview && (
            <div className="flex-shrink-0 px-8 py-3 bg-[var(--agent-subtle)] border-b border-[var(--agent-border)]">
              <div className="flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-[var(--agent-light)]" />
                <span className="text-sm font-medium text-[var(--agent-light)]">Agent submitted work for review</span>
                <span className="text-xs text-[var(--foreground-tertiary)] ml-auto">Approve or send feedback below</span>
              </div>
            </div>
          )}
            ...
            {isReview && task.review_notes && (
              <div className="mb-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div>
                    <span className="text-sm font-medium text-[var(--agent-light)]">{assignedAgent?.name || "Agent"}</span>
                    <span className="text-xs text-[var(--foreground-tertiary)] ml-2">{relativeTime(task.updated)}</span>
                  </div>
                </div>
                <div className="deliverable-block ml-10">
                  {task.review_notes}
                </div>
              </div>
            )}
            ...
            <div className="chat-thread">
              {commentsLoading ? (
                  ...
                  return (
                    <div key={comment.id} className={`chat-message ${isUser ? "user" : ""}`}>
                      <div className={`chat-avatar ${isUser ? "user" : isAgent ? "agent" : "system"}`}>
                        {isUser ? "DW" : isAgent ? <Bot className="h-3.5 w-3.5" /> : <GitBranch className="h-3 w-3" />}
                      </div>
                      <div className="flex flex-col min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs font-medium ${isUser ? "text-[var(--primary-light)]" : isAgent ? "text-[var(--agent-light)]" : "text-[var(--foreground-tertiary)]"}`}>
                            {comment.author}
                          </span>
                          ...
                          {comment.comment_type === "reassignment" && (
                            <span className="text-[10px] font-medium text-[var(--info)] bg-[var(--info-subtle)] px-1.5 py-0.5 rounded">
                              reassigned
                            </span>
                          )}
                          <span className="text-[11px] text-[var(--foreground-quaternary)]">{relativeTime(comment.created)}</span>
                        </div>
                ...
                <div className="flex flex-col items-center py-16 text-center">
                  <MessageSquare className="h-7 w-7 text-[var(--foreground-quaternary)] mb-3" />
                  <p className="text-sm text-[var(--foreground-tertiary)]">No comments yet</p>
                  <p className="text-xs text-[var(--foreground-quaternary)] mt-1">{isReview ? "Send feedback to the agent" : "Start a conversation"}</p>
                </div>
              )}
          ...
          {/* Input Area */}
          <div className="flex-shrink-0 border-t border-[var(--border)] px-8 py-3">
            <div className="flex gap-2 items-end">
              <div className="flex-1">
                <textarea
                  ...
                  rows={1}
                  className="w-full rounded-md bg-[var(--surface)] border border-[var(--border)] px-4 py-3 text-sm text-[var(--foreground)] placeholder:text-[var(--foreground-quaternary)] focus:outline-none focus:border-[var(--primary-border)] focus:bg-[var(--surface-elevated)] transition-colors resize-none overflow-hidden"
                  onKeyDown={(e) => {
              ...
              <button
                ...
                className="flex items-center justify-center rounded-md bg-[var(--primary)] hover:bg-[var(--primary-dark)] text-white px-4 py-3 transition-colors disabled:opacity-40"
              >
            ...
            {isReview && (
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[var(--border)]">
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-md bg-[var(--success)] hover:opacity-90 text-[var(--background)] font-medium px-4 py-2 text-sm transition-opacity"
                >
                  <Check className="h-4 w-4" /> Approve
                </button>
                <button
                  onClick={handleReject}
                  className="flex items-center justify-center gap-2 rounded-md border border-[var(--border)] text-[var(--foreground-secondary)] hover:bg-[var(--surface-hover)] hover:border-[var(--border-hover)] px-4 py-2 text-sm transition-colors"
                >
        ...
        {/* Properties Sidebar */}
        <div className="hidden lg:flex w-[260px] flex-shrink-0 border-l border-[var(--border)] bg-[var(--surface)] flex-col">
          {/* Assignee */}
          <div className="px-5 py-4 border-b border-[var(--border)]">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="text-[10px] font-medium text-[var(--primary)] hover:text-[var(--primary-light)] transition-colors"
              >
                Reassign
              </button>
            </div>
            {assignedAgent ? (
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-[var(--agent-subtle)] border border-[var(--agent-border)] flex items-center justify-center">
                ...
                <div className={`ml-auto h-2 w-2 rounded-full ${assignedAgent.paused ? "bg-[var(--foreground-tertiary)]" : "bg-[var(--success)]"}`} />
              </div>
            ) : (
            ...
            {reassignOpen && (
              <div className="mt-3 pt-3 border-t border-[var(--border)]">
                <CustomSelect label="" value={newAssignee} options={agentOptions} onChange={(v) => setNewAssignee(v)} />
                <div className="flex gap-2 mt-2">
                  <button
                    onClick={() => { setReassignOpen(false); setNewAssignee(""); }}
                    className="flex-1 rounded-md border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--foreground-secondary)] hover:bg-[var(--surface-hover)] transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 rounded-md bg-[var(--primary)] hover:bg-[var(--primary-dark)] text-white font-medium px-3 py-1.5 text-xs transition-colors disabled:opacity-40"
                  >
            ...
            <div className="space-y-0">
              ...
              {task.revision_count !== undefined && task.revision_count > 0 && (
                <div className="property-row">
                  ...
                  <span className="text-sm text-[var(--agent-light)] font-medium">{task.revision_count}</span>
                </div>
              )}
              {project && (
                <div className="property-row">
                  <span className="property-label">Project</span>
                  <Link href={`/projects/${project.id}`} className="text-sm text-[var(--primary)] hover:underline">
                    {project.name}
                  </Link>
                </div>
              )}
